...
function default_front_page_posts()
{
	...
	$lon_max = $lon+$radius;
	$lon_min = $lon-$radius;
	$query = "SELECT id, title, date, photo, tag_1, tag_2, tag_3, lat, lon FROM postings WHERE active=1 AND $lat_min <= lat AND lat <= $lat_max AND ($lon_min) <= lon AND lon <= ($lon_max)";
